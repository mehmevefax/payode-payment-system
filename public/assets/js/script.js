...
(function () {
    ...
    // Datatable -- BU KISIM TEMADAN GELEN OTOMATİK BAŞLATMAYI İPTAL ETTİK
    // BURADA DataTables ile ilgili KOD OLMAMALI
    /*
    ...
    */
    ...
    // Düzeltme: toggle-passworda içindeki fazla "setTimeout" kelimesini kaldırdık
    ...
    // Sidebar (Colsidemenu - ikincil sidebar)
    ...
    // Date Range Picker (bookingrange)
    ...
    if($('.daterange').length > 0) {
        ...
        $('#daterange').on('input', function() {
            ...
            $(this).css('width', (textLength + 10) + 'px');
        });
    ...
    // toggle_btn (sidebar küçültme)
    $(document).on('click', '#toggle_btn', function() {
        if ($('body').hasClass('mini-sidebar')) {
            $('body').removeClass('mini-sidebar');
            $(this).addClass('active');
            $('.subdrop + ul');
            localStorage.setItem('screenModeNightTokenState', 'night'); // Tema modu ayarı olabilir
            setTimeout(function() {
                $("body").removeClass("mini-sidebar");
            ...
            $('.subdrop + ul');
            localStorage.removeItem('screenModeNightTokenState', 'night'); // Tema modu ayarı olabilir
            setTimeout(function() {
                $("body").addClass("mini-sidebar");
    ...
    // Theme color/layout settings (local storage'ı kontrol eden kısım)
    ...
    // Checkbox select all (datatable'larda)
    ...
    // Light/Dark mode localStorage ayarı
    ...
    // Custom Input Range (CSS gradient ile)
    ...
    // Select All Checkbox
    var selectAllItems = "#select-all"; // Burada temasal bir çakışma olabilir (select-all2'den select-all'a dönmüşüm)
    var checkboxItem = ":checkbox"; // Genel bir seçici, datatables içindeki checkboxları da kapsar
    $(selectAllItems).on('click', function(){
        if (this.checked) {
            $(checkboxItem).each(function() {
                this.checked = true;
    ...
    // Global toggleFullscreen fonksiyonu (tekrar tanımlanmadan sadece bir kez tanımlanmalı)
...
})(); // Bu parantez JavaScript'teki IIFE'nin (Immediately Invoked Function Expression) kapanışıdır
