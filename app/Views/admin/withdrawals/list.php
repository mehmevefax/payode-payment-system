<?php
// app/Views/admin/withdrawals/list.php
namespace App\Views\Admin\Withdrawals; // Bu sayfa için özel isim alanı
...
// $withdrawals, $pagination, $filters, $pageTitle, $currentSection, $activeMenu, $activeMethodSlug, $activeMenuSub
// $pending_withdrawals_count, $rejected_withdrawals_count, $total_approved_withdrawals_amount
?>
<?php ob_start(); // Sayfa çıktısını tamponlamaya başla ?>

<style>
    /* Tablo ve kapsayıcıları için doğrudan ve güçlü CSS kuralları (deposits/list.php'deki ile aynı veya benzer) */
    ...
    .withdrawal-table tbody tr:nth-of-type(odd) {
    ...
    .withdrawal-table th:nth-child(2) { width: 120px; } /* Ref ID */
    .withdrawal-table th:nth-child(3) { width: 150px; } /* Kullanıcı */
    .withdrawal-table th:nth-child(4) { width: 100px; } /* Miktar */
    .withdrawal-table th:nth-child(5) { width: 120px; } /* Yöntem */
    .withdrawal-table th:nth-child(6) { width: 100px; } /* Durum */
    .withdrawal-table th:nth-child(7) { width: 150px; } /* İşlem Tarihi */
    .withdrawal-table th:nth-child(8) { width: 120px; } /* Onay Süresi */
    .withdrawal-table th:nth-child(9) { width: 120px; } /* Onaylayan */
...
<div class="withdrawal-list-section">
            ...
            <div class="me-2 mb-2">
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end p-3">
                        <li><a href="<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=export_pdf&<?= http_build_query($filters) ?>" class="dropdown-item rounded-1"><i class="ti ti-file-type-pdf me-1"></i>PDF Olarak Aktar</a></li>
                        <li><a href="<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=export_excel&<?= http_build_query($filters) ?>" class="dropdown-item rounded-1"><i class="ti ti-file-type-xls me-1"></i>Excel Olarak Aktar</a></li>
                    </ul>
                </div>
            </div>
            <div class="mb-2">
                <a href="#" data-bs-toggle="modal" data-bs-target="#add_withdrawal_modal" class="btn btn-primary d-flex align-items-center"><i class="ti ti-circle-plus me-2"></i>Yeni Çekim Ekle</a>
            </div>
                ...
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center overflow-hidden">
                        ...
                        <div class="ms-2 overflow-hidden">
                            <p class="fs-12 fw-medium mb-1 text-truncate">Toplam Çekim İşlemi</p>
                ...
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center overflow-hidden">
                        ...
                        <div class="ms-2 overflow-hidden">
                            ...
                            <h4><?= $pending_withdrawals_count ?? 0 ?></h4>
                        </div>
                    </div>
                ...
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center overflow-hidden">
                        ...
                        <div class="ms-2 overflow-hidden">
                            ...
                            <h4>$<?= number_format($total_approved_withdrawals_amount ?? 0, 2) ?></h4>
                        </div>
                    </div>
                ...
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center overflow-hidden">
                        ...
                        <div class="ms-2 overflow-hidden">
                            ...
                            <h4><?= $rejected_withdrawals_count ?? 0 ?></h4>
                        </div>
                    </div>
    ...
    <div class="card main-card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
            <h5>Para Çekme İşlemleri Listesi</h5>
        ...
        <div class="card-body p-0">
            <div class="withdrawal-table-wrapper">
                <table class="table table-hover table-striped withdrawal-table">
                        ...
                        <?php if (!empty($withdrawals)): ?>
                            <?php foreach ($withdrawals as $withdrawal): ?>
                                <tr>
                                    <td>
                                        <div class="form-check form-check-md">
                                            <input class="form-check-input" type="checkbox" value="<?= $withdrawal['id'] ?>">
                                        </div>
                                    </td>
                                    <td><?= htmlspecialchars($withdrawal['ref_id']) ?></td>
                                    <td><?= htmlspecialchars($withdrawal['user_username']) ?> (<?= htmlspecialchars($withdrawal['user_namesurname_full']) ?>)</td>
                                    <td><?= number_format($withdrawal['amount'], 2) ?> <?= htmlspecialchars($withdrawal['currency']) ?></td>
                                    <td><?= htmlspecialchars($withdrawal['payment_method_name'] ?? 'Bilinmiyor') ?></td>
                                    <td>
                                        <?php
                                            $statusClass = '';
                                            if ($withdrawal['status'] == 'pending') $statusClass = 'badge bg-warning';
                                            elseif ($withdrawal['status'] == 'approved') $statusClass = 'badge bg-success';
                                            elseif ($withdrawal['status'] == 'rejected') $statusClass = 'badge bg-danger';
                                            elseif ($withdrawal['status'] == 'cancelled') $statusClass = 'badge bg-info';
                                        ?>
                                        <span class="<?= $statusClass ?> d-inline-flex align-items-center badge-xs">
                                            <i class="ti ti-point-filled me-1"></i><?= ucfirst($withdrawal['status']) ?>
                                        </span>
                                    </td>
                                    <td><?= htmlspecialchars(date('d M Y H:i:s', strtotime($withdrawal['transaction_date']))) ?></td>
                                    <td>
                                        <?php if ($withdrawal['processing_time_seconds'] !== null): ?>
                                            <?= htmlspecialchars(floor($withdrawal['processing_time_seconds'] / 60)) ?> dk <?= htmlspecialchars($withdrawal['processing_time_seconds'] % 60) ?> sn
                                        <?php else: ?>
                                            N/A
                                        <?php endif; ?>
                                    </td>
                                    <td><?= htmlspecialchars($withdrawal['approved_by_username'] ?? 'Beklemede') ?></td>
                                    <td>
                                        <div class="action-icon d-inline-flex">
                                            <a href="#" class="me-2 btn btn-sm btn-info view-withdrawal-detail" data-bs-toggle="modal" data-bs-target="#withdrawal_detail_modal" data-id="<?= $withdrawal['id'] ?>" title="Detay Görüntüle"><i class="ti ti-eye"></i></a>
                                            <?php if ($withdrawal['status'] === 'pending'): ?>
                                                <a href="javascript:void(0);" class="me-2 btn btn-sm btn-success btn-approve-withdrawal" data-id="<?= $withdrawal['id'] ?>" title="Onayla"><i class="ti ti-check"></i></a>
                                                <a href="javascript:void(0);" class="me-2 btn btn-sm btn-danger btn-reject-withdrawal" data-id="<?= $withdrawal['id'] ?>" title="Reddet"><i class="ti ti-x"></i></a>
                                            <?php endif; ?>
                                            <a href="javascript:void(0);" class="btn btn-sm btn-secondary btn-delete-withdrawal" data-id="<?= $withdrawal['id'] ?>" data-bs-toggle="modal" data-bs-target="#delete_withdrawal_modal" title="Sil"><i class="ti ti-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="d-flex justify-content-end p-3">
                <ul class="pagination mb-0">
                    <?php if (($pagination['current_page'] ?? 1) > 1): ?>
                        <li class="page-item"><a class="page-link" href="?page=withdrawals&p=<?= ($pagination['current_page'] - 1) ?>&sayfada=<?= htmlspecialchars($pagination['limit']) ?>&search=<?= htmlspecialchars($filters['search'] ?? '') ?>&status=<?= htmlspecialchars($filters['status'] ?? '') ?>&method_slug=<?= htmlspecialchars($filters['method_slug'] ?? '') ?>">Önceki</a></li>
                    <?php endif; ?>
                    <?php for ($i = 1; $i <= ($pagination['total_pages'] ?? 1); $i++): ?>
                        <li class="page-item <?= (($pagination['current_page'] ?? 1) == $i ? 'active' : '') ?>">
                            <a class="page-link" href="?page=withdrawals&p=<?= $i ?>&sayfada=<?= htmlspecialchars($pagination['limit']) ?>&search=<?= htmlspecialchars($filters['search'] ?? '') ?>&status=<?= htmlspecialchars($filters['status'] ?? '') ?>&method_slug=<?= htmlspecialchars($filters['method_slug'] ?? '') ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>
                    <?php if (($pagination['current_page'] ?? 1) < ($pagination['total_pages'] ?? 1)): ?>
                        <li class="page-item"><a class="page-link" href="?page=withdrawals&p=<?= ($pagination['current_page'] + 1) ?>&sayfada=<?= htmlspecialchars($pagination['limit']) ?>&search=<?= htmlspecialchars($filters['search'] ?? '') ?>&status=<?= htmlspecialchars($filters['status'] ?? '') ?>&method_slug=<?= htmlspecialchars($filters['method_slug'] ?? '') ?>">Sonraki</a></li>
                    <?php endif; ?>
                </ul>
            </div>
        </div>
    ...
    <div class="modal fade" id="add_withdrawal_modal" tabindex="-1" aria-labelledby="add_withdrawal_modal_label" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="add_withdrawal_modal_label">Yeni Para Çekme İşlemi Ekle</h5>
                    <button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                <form id="add_withdrawal_form" action="<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=add" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="method_id" class="form-control select2" required>
                                    ...
                                    $withdrawalMethods = $paymentMethodModel->getAllPaymentMethods(['method_type' => 'withdrawal']);
                            ...
                            <div class="col-md-6 mb-3">
                             ...
                             <div class="col-md-6 mb-3">
                                <label class="form-label">Hedef Hesap Detayları (JSON) <span class="text-danger">*</span></label>
                                <textarea name="target_account_details" class="form-control" rows="1" placeholder='Örnek: {"iban": "TRXX...", "bank_name": "Garanti", "account_holder": "Ad Soyad"}' required></textarea>
                                <small class="form-text text-muted">IBAN, Papara No, Kripto Adresi gibi detayları JSON formatında giriniz.</small>
                            </div>
                             <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                             ...
                             <div class="col-md-12 mb-3">
                                ...
                                <textarea name="client_info" class="form-control" rows="2" placeholder='Örnek: {"user_agent": "...", "locale": "tr"}'></textarea>
                                <small class="form-text text-muted">API'den gelen ek veriler için.</small>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light me-2" data-bs-dismiss="modal">İptal</button>
                        <button type="submit" class="btn btn-primary">Çekim Ekle</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
    <div class="modal fade" id="withdrawal_detail_modal" tabindex="-1" aria-labelledby="withdrawal_detail_modal_label" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="withdrawal_detail_modal_label">Para Çekme İşlem Detayı</h5>
                    <button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div id="withdrawal_detail_content">
    ...
    <div class="modal fade" id="delete_withdrawal_modal" tabindex="-1" aria-labelledby="delete_withdrawal_modal_label" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="delete_withdrawal_modal_label">İşlemi Silmek İstiyor Musunuz?</h5>
                    <button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                <div class="modal-body text-center">
                    ...
                    <div class="d-flex justify-content-center">
                        ...
                        <button type="button" class="btn btn-danger confirm-delete-withdrawal-btn">Evet, Sil</button>
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    if ($.fn.select2) {
        $('#add_withdrawal_modal .select2').select2({
            dropdownParent: $('#add_withdrawal_modal'),
            width: '100%'
        });
    }

    // Export butonları için JavaScript (şimdilik CSV olarak indirme)
    $(document).on('click', '.export-pdf-btn', function(e) {
        e.preventDefault();
        const currentUrlParams = new URLSearchParams(window.location.search);
        currentUrlParams.set('action', 'export_pdf');
        // Export işlemi için yeni bir pencere açmak daha iyidir
    ...
    $(document).on('click', '.export-excel-btn', function(e) {
        e.preventDefault();
        const currentUrlParams = new URLSearchParams(window.location.search);
        currentUrlParams.set('action', 'export_excel');
        window.open('<?= Config::BASE_URL ?>/admin.php?' + currentUrlParams.toString(), '_blank');
    });


    // Add Withdrawal Form Submit
    $(document).on('submit', '#add_withdrawal_form', function(e) {
        e.preventDefault();
        const form = $(this);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    $('#add_withdrawal_modal').modal('hide');
                    location.reload();
                } else {
    ...
    $(document).on('click', '.view-withdrawal-detail', function() {
        const withdrawalId = $(this).data('id');
        $('#withdrawal_detail_content').html('<p class="text-center">Yükleniyor...</p>');
        $.ajax({
            url: '<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=view_detail',
            type: 'GET',
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    const targetAccountDetailsPretty = JSON.stringify(detail.target_account_details, null, 2);
                    ...
                    let htmlContent = `
                        <div class="p-3">
                            ...
                            <div class="pb-1 border-bottom mb-4">
                                ...
                                <div class="row align-items-center">
                                     <div class="col-md-12 mb-3">
                                        <p class="fs-12 mb-0">Hedef Hesap Detayları</p>
                                        <p class="text-gray-9"><pre>${targetAccountDetailsPretty}</pre></p>
                                    </div>
                                </div>
                            </div>
                    ...
                    `;
                    $('#withdrawal_detail_content').html(htmlContent);
                } else {
                    $('#withdrawal_detail_content').html('<p class="text-danger text-center">' + (response.message || 'Detaylar yüklenirken bir hata oluştu.') + '</p>');
                }
            },
            error: function(xhr, status, error) {
                console.error("AJAX Error: ", status, error, xhr.responseText);
                $('#withdrawal_detail_content').html('<p class="text-danger text-center">Detaylar yüklenemedi. Sunucu hatası.</p>');
            }
        });
    });

    // Delete butonu için ID'yi modal'a aktar
    let withdrawalToDeleteId = 0;
    $(document).on('click', '.btn-delete-withdrawal', function() {
        withdrawalToDeleteId = $(this).data('id');
    });

    // Delete onayı butonu AJAX ile
    $(document).on('click', '.confirm-delete-withdrawal-btn', function() {
        $.ajax({
            url: '<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=delete',
            type: 'POST',
            data: { id: withdrawalToDeleteId, csrf_token: '<?= Session::generateCsrfToken() ?>' },
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    alert(response.message);
                    $('#delete_withdrawal_modal').modal('hide');
                    location.reload();
                } else {
    ...
    $(document).on('click', '.btn-approve-withdrawal', function() {
        ...
        if (confirm('Bu işlemi onaylamak istediğinize emin misiniz?')) {
            $.ajax({
                type: 'POST',
                url: '<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=approve',
    ...
    $(document).on('click', '.btn-reject-withdrawal', function() {
        ...
        if (reason !== null && reason.trim() !== '') {
            if (confirm('Bu işlemi reddetmek istediğinize emin misiniz?')) {
                $.ajax({
                    type: 'POST',
                    url: '<?= Config::BASE_URL ?>/admin.php?page=withdrawals&action=reject',
